...
= "/* Not a comment. */";" will not be a test case. (Also, nothing else such as defines or macros will interfere with
the comments.)
...
characters.  After deletion, the implicit newline characters are deleted, leaving the string "ab", which when delimited
...
class Solution:
    def removeComments(self, source):
        ...
        for i in range(len(source)):
            ...
            while j<len(line):
                char = line[j]
                if char=='/' and j<len(line)-1 and line[j+1] == '/' and blockComment == 0:
                    j=len(line)
                elif char=='/' and j<len(line)-1 and line[j+1] == '*' and blockComment == 0:
                    blockComment += 1
                    j+=1
                elif char == '*' and j < len(line) - 1 and line[j + 1] == '/' and blockComment > 0:
                    blockComment -= 1
                    ...
                    resLine += char
                j+=1
            if len(resLine)>0 and blockComment==0:
                res.append(resLine)
                resLine = ''
...
s = Solution()
#k = ["a/*comment", "line", "more_comment*/b"]
k = ["/*Test program */", "int main()", "{ ", "  // variable declaration ", "int a, b, c;", "/* This is a test", "   multiline  ", "   comment for ", "   testing */", "a = b + c;", "}"]
print(s.removeComments(k))
