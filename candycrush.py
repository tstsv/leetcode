    ...
    def candyCrush(self, board):
        ...
        while True:
            for i in range(h):
                for j in range(w):
                    if i>0 and i<h-1:
                        if board[i-1][j]==board[i][j] and board[i+1][j]==board[i][j] and board[i][j]!=0:
                            pop.update([(i,j),(i+1,j),(i-1,j)])

                    if j>0 and j<w-1:
                        if board[i][j-1]==board[i][j] and board[i][j+1]==board[i][j] and board[i][j]!=0:
                            pop.update([(i,j),(i,j-1),(i,j+1)])
            ...
            for j in range(w):
                ...
                for i in reversed(range(h)):
                    if board[i][j]!=0:
                        board[i][j],board[trail][j]=board[trail][j],board[i][j]
                        trail-=1
        return board
